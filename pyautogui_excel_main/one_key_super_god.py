...
def Analysis(PicName, location):
    ...
    while local < Key_Value_pair and running == 1:
        ...
        elif NowRowKey[local] == '按键长按':
            pyautogui.keyDown(str(NowRowValue[local]).split(':')[0])
            time.sleep(float(str(NowRowValue[local]).split(':')[1]))
            pyautogui.keyUp(str(NowRowValue[local]).split(':')[0])
